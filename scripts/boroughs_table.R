...
set_boroughs_table <- function() {
    ...
    colnames(data) <- c("Borough","Region of Birth","Country of Birth",
                        "Count","Percentage")
    ...
    oth_ind <- grep("other",tab_dat$`Country of Birth`, ignore.case = TRUE)
    ...
    tab_dat <- tab_dat[,-"tag"]
    save(tab_dat, file = "report_data/RDA/boroughs_table.RData")
    
    
}

get_boroughs_table <- function() {
    load("report_data/RDA/boroughs_table.RData")
    font.size <- "9.5pt"
    datatable(tab_dat,
              extensions = "Buttons",
              rownames = "",
